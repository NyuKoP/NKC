...
export default function App() {
  ...
  const hydrateVault = useCallback(async () => {
    try {
      ...
      addToast({ message: "??션??만료??었??니?? ??시 로그??해 주세??" });
    }
  }, [
    addToast,
    devLog,
  ...
  // ??cleanup ????문제(EffectCallback) + unsubscribe ????방어
  useEffect(() => {
    if (typeof window !== "undefined") {
  ...
  const handleCreate = async (displayName: string) => {
    ...
    try {
      ...
      setOnboardingError(error instanceof Error ? error.message : "금고 초기??에 ??패??습??다.");
      lockVault();
      addToast({ message: "금고 초기??에 ??패??습??다." });
    } finally {
  ...
  const handleImport = async (recoveryKey: string, displayName: string) => {
    ...
    if (!validateRecoveryKey(recoveryKey)) {
      addToast({ message: "복구????식????바르?? ??습??다. (?? NKC-...)" });
      onboardingLockRef.current = false;
      return;
    }
    ...
    try {
      ...
      lockVault();
      addToast({ message: "복구??로 가??오기에 ??패??습??다." });
    } finally {
      onboardingLockRef.current = false;
    }
  };
  ...
  const handleGenerateRecoveryKey = async (newKey: string) => {
    try {
      ...
      addToast({ message: "복구???? 변경되??습??다. PIN????시 ??정??주세??" });
    } catch (error) {
      console.error("Failed to rotate recovery key", error);
      addToast({ message: "복구??변경에 ??패??습??다." });
      throw error;
    }
  };
  ...
  const handleFriendToggleFavorite = async (friendId: string) => {
    ...
    try {
      ...
      addToast({ message: "즐겨찾기 변경에 ??패??습??다." });
    }
  };
  ...
  const handleAddFriend = async (payload: { code: string; psk?: string }) => {
    ...
    const routingHints =
      decoded.onionAddr || decoded.alternateRouteAddr
        ? { onionAddr: decoded.onionAddr, alternateRouteAddr: decoded.alternateRouteAddr }
        : undefined;
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          // confirm?.onConfirm() ??Promise??반환??도 UI??void??처리
