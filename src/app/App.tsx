...
export default function App() {
  ...
  const resolveLocalRoutingHintsForFriendCode = useCallback(async () => {
    ...
    let alternateRouteAddr = fallback?.alternateRouteAddr;
  ...
  }, [userProfile?.routingHints?.alternateRouteAddr, userProfile?.routingHints?.onionAddr]);
  ...
  const friendAddHint = useMemo(() => {
    ...
    const selectedStatus =
      selectedNetwork === "tor" ? netConfig.tor.status : netConfig.alternateRoute.status;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        if (onionActive) {
          ...
          if (netConfig.onionSelectedNetwork === "tor") {
          ...
          } else if (netConfig.onionSelectedNetwork === "alternateRoute") {
            const alternateRouteBefore = nkc?.getalternateRouteStatus ? await nkc.getalternateRouteStatus() : null;
            runtimeBootstrap.alternateRouteBefore = toRuntimeState(alternateRouteBefore);
            runtimeBootstrap.alternateRouteBeforeDetail = toRuntimeDetail(alternateRouteBefore);
            if (nkc?.startalternateRoute && toRuntimeState(alternateRouteBefore) !== "running") {
              try {
            ...
            const alternateRouteAfter = nkc?.getalternateRouteStatus ? await nkc.getalternateRouteStatus() : null;
            runtimeBootstrap.alternateRouteAfter = toRuntimeState(alternateRouteAfter);
            runtimeBootstrap.alternateRouteAfterDetail = toRuntimeDetail(alternateRouteAfter);
          }
        }
    ...
    emitRouterTestLog,
    netConfig,
    netConfig.alternateRoute.status,
    netConfig.mode,
  ...
  const sendFriendControlPacket = useCallback(
    async (
      ...
      const shouldPreferOnionRouterForFriendControl =
        ...
        Boolean(routingMeta.route?.torOnion || routingMeta.route?.alternateRoute);
