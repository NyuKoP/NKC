...
export const sha256 = (message: Uint8Array) => {
  const bitLengthHi = Math.floor((message.length * 8) / 0x1_0000_0000);
  const bitLengthLo = (message.length * 8) >>> 0;
  const totalLength = ((message.length + 9 + 63) >>> 6) << 6;
  const padded = new Uint8Array(totalLength);
  padded.set(message);
  padded[message.length] = 0x80;
  writeUint32Be(padded, totalLength - 8, bitLengthHi);
  writeUint32Be(padded, totalLength - 4, bitLengthLo);
