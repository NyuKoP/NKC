...
describe("handleOnionSend routing", () => {
  it("auto: alternateRoute fails then tor succeeds", async () => {
    ...
    deps.socksFetch.mockImplementation(async (url: string) => {
      if (url.includes("alternateRoute")) {
    ...
    const result = await handleOnionSend(
      {
        toDeviceId: "peer-1",
        envelope: "env",
        route: { mode: "preferalternateRoute", alternateRoute: "alternateRoute.test", torOnion: "abc.onion" },
      },
      deps
    );
    ...
    const result = await handleOnionSend(
      {
        toDeviceId: "peer-1",
        envelope: "env",
        route: { mode: "preferTor", alternateRoute: "alternateRoute.test", torOnion: "abc.onion" },
      },
      deps
    );
    expect(result.body.ok).toBe(false);
