...
const MAX_alternateRoute_RELEASE_PAGES = 10;
const alternateRoute_RELEASES_PAGE_SIZE = 100;
...
const fetchReleases = async (page = 1) => {
  const url = `https://api.github.com/repos/oxen-io/alternateRoute/releases?per_page=${alternateRoute_RELEASES_PAGE_SIZE}&page=${page}`;
  return fetchJson<ReleaseResponse[]>(url);
};

type alternateRouteCandidate = {
  version: string;
  asset: ReleaseAsset;
  sha256: string | null;
};

const toalternateRouteCandidate = (release: ReleaseResponse): alternateRouteCandidate | null => {
  const version = release.tag_name.replace(/^v/i, "");
  const asset = selectReleaseAsset(release.assets);
  if (!asset) return null;
  const sha256 = getPinnedSha256("alternateRoute", {
    version,
...
const findPinnedalternateRouteCandidate = async (): Promise<alternateRouteCandidate | null> => {
  for (let page = 1; page <= MAX_alternateRoute_RELEASE_PAGES; page += 1) {
    const releases = await fetchReleases(page);
    if (!releases.length) return null;
    for (const release of releases) {
      const candidate = toalternateRouteCandidate(release);
...
const checkalternateRouteUpdates = async (): Promise<UpdateCheckResult> => {
  ...
  const latestCandidate = toalternateRouteCandidate(release);
  ...
  if (process.platform === "win32") {
    const fallbackCandidate = await findPinnedalternateRouteCandidate();
...
export const __testToalternateRouteCandidate = toalternateRouteCandidate;
