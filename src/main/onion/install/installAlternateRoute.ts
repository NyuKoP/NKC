...
import { getalternateRouteAssetName, getalternateRouteAssetUrlForName } from "../assetNaming";
...
const ensureExecutable = async (binaryPath: string) => {
  ...
  try {
    ...
    console.warn("[onion] failed to chmod alternateRoute binary", {
      binaryPath,
...
const findalternateRouteBinaryWin32 = async () => {
  ...
  await new Promise<void>((resolve) => {
    execFile("where.exe", ["alternateRoute.exe"], { windowsHide: true }, (_error, stdout) => {
      if (stdout) {
        candidates.push(
          ...stdout
            .toString()
...
export const installalternateRoute = async (
  userDataDir: string,
  version: string,
  ...
  const network: OnionNetwork = "alternateRoute";
