...
export const sendCiphertext = async (
  ...
  emitFlowTraceLog({
    ...
    attempted: [chosen],
    fallbackUsed: false,
    policy: config.mode === "onionRouter" || config.onionEnabled ? "STRICT" : "ALLOW_FALLBACK",
...
export const sendOutboxRecord = async (
  ...
  const resolve = deps.resolveTransport ?? resolveTransport;
  const chosen = resolve(config, controller);
  emitRouteGateCheck(record.id, chosen, config, torOnion || alternateRoute ? { torOnion, alternateRoute } : undefined);
  emitFlowTraceLog({
    event: "routeSelect:decision",
    opId: record.id,
    wanted: chosen,
