...
export default function SettingsDialog({
  ...
  const alternateRouteState = netConfig.alternateRoute;
    ...
    const syncStatus = async () => {
      try {
        ...
        setOnionStatus({
          ...
          alternateRoute: status.components.alternateRoute,
            ...
            <div className="mt-6 grid gap-6">
              ...
              <section className="rounded-nkc border border-nkc-border bg-nkc-panelMuted p-6">
                <div className="grid gap-4 text-sm">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold text-nkc-text">Onion Router (익명)</div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      {([
                        ["tor", "Tor", torState],
                        ["alternateRoute", "alternateRoute", alternateRouteState],
                          ...
                          >
                            <div>
                              ...
                              <div className="text-[11px] text-nkc-muted">
                                ...
                                {state.latest && state.version && compareVersions(state.latest, state.version) > 0
                                  ? ` · 최신 v${state.latest}`
                                  : ""}
                              </div>
                            </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <label className="flex items-center justify-between text-sm">
                        <span>alternateRoute</span>
                        <input
                          type="radio"
                          name="onion-network"
                          checked={onionNetworkDraft === "alternateRoute"}
                          onChange={() => setOnionNetworkDraft("alternateRoute")}
                          disabled={!alternateRouteState.installed}
                        />
                      </label>
                    </div>
                    <div className="text-xs text-nkc-muted leading-relaxed">
                      <p>설치된 Tor/alternateRoute을 로컬에서 실행해 SOCKS 프록시로 연결합니다.</p>
                      <p>Onion 사용 시 direct P2P는 차단되며, 연결 상태에 따라 전송이 지연될 수 있습니다.</p>
                    </div>
                    {onionEnabledDraft &&
                    !(onionNetworkDraft === "tor" ? torState.installed : alternateRouteState.installed) ? (
                      <div className="text-xs text-red-300">
                        선택한 네트워크가 설치되지 않았습니다.
                      </div>
                    ) : null}
