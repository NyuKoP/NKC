...
type ConnectionChoice = "directP2P" | "selfOnion" | "torOnion" | "alternateRouteOnion";
...
export default function SettingsDialog({
  ...
  const handleConnectionChoiceChange = async (choice: ConnectionChoice) => {
    ...
    setMode("onionRouter");
    setOnionNetwork("alternateRoute");
  ...
  const connectionChoice: ConnectionChoice =
    netConfig.mode === "onionRouter"
      ? netConfig.onionSelectedNetwork === "alternateRoute"
        ? "alternateRouteOnion"
        : "torOnion"
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3">
                        <input
                          ...
                          onChange={() => void handleConnectionChoiceChange("alternateRouteOnion")}
                          ...
                          {!netConfig.alternateRoute.installed ? (
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    await refreshOnionStatus();
                                    setSaveMessage(
                                      t("alternateRoute 상태 확인 완료", "alternateRoute status checked")
                                    );
                                  } catch (error) {
                                    console.error("Failed to refresh alternateRoute status", error);
                                    setSaveMessage(
                                      t("alternateRoute 상태 확인 실패", "alternateRoute status check failed")
                                    );
                                  } finally {
                            ...
                            <>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  alternateRouteUpdateAvailable
                                    ? void handleApplyUpdate("alternateRoute")
