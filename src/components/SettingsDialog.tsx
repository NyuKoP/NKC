...
export default function SettingsDialog({
  ...
  const handleInstall = async (network: "tor" | "alternateRoute") => {
    ...
    try {
      ...
      setSaveMessage(network === "tor" ? "Tor 설치 완료" : "alternateRoute 설치 완료");
    } catch (error) {
      console.error("Failed to install onion component", error);
      setSaveMessage(network === "tor" ? "Tor 설치 실패" : "alternateRoute 설치 실패");
    } finally {
  ...
  const handleUninstall = async (network: "tor" | "alternateRoute") => {
    ...
    try {
      ...
      await refreshOnionStatus();
      setSaveMessage(network === "tor" ? "Tor 제거 완료" : "alternateRoute 제거 완료");
    } catch (error) {
      console.error("Failed to uninstall onion component", error);
      setSaveMessage(network === "tor" ? "Tor 제거 실패" : "alternateRoute 제거 실패");
    } finally {
  ...
  const alternateRouteUpdateStatus = netConfig.lastUpdateCheckAtMs
    ? netConfig.alternateRoute.error === "PINNED_HASH_MISSING"
      ...
      : alternateRouteUpdateAvailable
        ? `업데이트 가능: ${netConfig.alternateRoute.latest}`
        : "최신 상태"
    : "";
                        ...
                        <div className="text-xs text-nkc-muted">
                          {opt.value === "directP2P"
                            ...
                            : opt.value === "onionRouter"
                              ? "외부 Onion 경로(Tor/alternateRoute)"
                              : "내장 Onion 경로(N hops)"}
                        </div>
                      </div>
                    </label>
                      ...
                      <div className="rounded-nkc border border-nkc-border bg-nkc-panel px-4 py-3">
                        <div className="flex items-center justify-between">
                          <label className="flex items-center gap-2 text-sm font-medium text-nkc-text">
                            ...
                            />
                            alternateRoute <span className="text-xs text-nkc-muted">⚠ 고급</span>
                          </label>
                          <div className="text-xs text-nkc-muted">
                        ...
                        {alternateRouteUpdateStatus ? (
                          <div className="mt-2 text-xs text-nkc-muted">{alternateRouteUpdateStatus}</div>
                        ) : null}
                        {alternateRouteErrorLabel ? (
                          <div className="mt-2 text-xs text-red-300">{alternateRouteErrorLabel}</div>
                        ) : null}
                        <div className="mt-3 flex flex-wrap gap-2">
                          {!netConfig.alternateRoute.installed ? (
                            <button
                              ...
                              className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panelMuted disabled:opacity-50"
                            >
                              {alternateRouteInstallBusy ? "처리 중..." : "alternateRoute 설치(관리자 권한 필요)"}
                            </button>
                          ) : null}
                          <button
                            type="button"
                            onClick={async () => {
                              ...
                              try {
                                await refreshOnionStatus();
                                setSaveMessage("alternateRoute 상태 확인 완료");
                              } catch (error) {
                                console.error("Failed to refresh alternateRoute status", error);
                                setSaveMessage("alternateRoute 상태 확인 실패");
                              } finally {
                            ...
                            className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panelMuted disabled:opacity-50"
                          >
                            {alternateRouteStatusBusy ? "처리 중..." : "상태 확인"}
                          </button>
                          ...
                          {alternateRouteUpdateAvailable ? (
                            <button
                              ...
                              className="rounded-nkc bg-nkc-accent px-3 py-2 text-xs font-semibold text-nkc-bg disabled:opacity-50"
                            >
                              {alternateRouteApplyBusy ? "처리 중..." : "업데이트 적용"}
                            </button>
                          ) : null}
                          {netConfig.alternateRoute.installed ? (
                            <button
                              ...
                              className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panelMuted disabled:opacity-50"
                            >
                              {alternateRouteUninstallBusy ? "처리 중..." : "제거"}
                            </button>
                          ) : null}
                        </div>
                      </div>
