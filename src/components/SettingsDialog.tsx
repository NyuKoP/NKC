...
import {
  ...
  getRoutePolicy,
  getalternateRouteExternalProxyUrl,
  getalternateRouteServiceAddress,
  setPrivacyPrefs,
  setRendezvousBaseUrl,
  setRendezvousUseOnion,
  setOnionControllerUrlOverride,
  setRoutePolicy,
  setalternateRouteExternalProxyUrl,
  setalternateRouteServiceAddress,
...
type RoutePolicyMode = "auto" | "preferalternateRoute" | "preferTor" | "manual";
...
const getNkcBridge = () =>
  (globalThis as {
    nkc?: {
      ...
      configurealternateRouteExternal?: (payload: { proxyUrl: string; serviceAddress?: string }) => Promise<unknown>;
      startalternateRoute?: () => Promise<unknown>;
      stopalternateRoute?: () => Promise<unknown>;
    };
  }).nkc;
...
export default function SettingsDialog({
  ...
  const [alternateRouteStatus, setalternateRouteStatus] = useState<alternateRouteStatus | null>(null);
  const [alternateRouteBusy, setalternateRouteBusy] = useState(false);
  const [alternateRouteProxyDraft, setalternateRouteProxyDraft] = useState("");
  const [alternateRouteServiceDraft, setalternateRouteServiceDraft] = useState("");
  ...
  const handleCheckOnionControllerHealth = async () => {
    ...
    try {
      ...
      if (health.alternateRoute?.address) {
        setalternateRouteServiceDraft(health.alternateRoute.address);
      }
    } catch (error) {
      setOnionControllerHealth({
        ok: false,
  ...
  const refreshalternateRouteStatus = async () => {
    const nkc = getNkcBridge();
    if (!nkc?.getalternateRouteStatus) return;
    try {
      const status = (await nkc.getalternateRouteStatus()) as alternateRouteStatus;
      setalternateRouteStatus(status);
      if (status.serviceAddress) {
        setalternateRouteServiceDraft(status.serviceAddress);
      }
    } catch (error) {
  ...
  const handleApplyalternateRouteExternal = async () => {
    const nkc = getNkcBridge();
    if (!nkc?.configurealternateRouteExternal) return;
    if (alternateRouteBusy) return;
    setalternateRouteBusy(true);
    try {
      await setalternateRouteExternalProxyUrl(alternateRouteProxyDraft);
      await setalternateRouteServiceAddress(alternateRouteServiceDraft);
      await nkc.configurealternateRouteExternal({
        proxyUrl: alternateRouteProxyDraft.trim(),
        serviceAddress: alternateRouteServiceDraft.trim() || undefined,
      });
      await refreshalternateRouteStatus();
    } catch (error) {
      console.error("Failed to configure alternateRoute", error);
    } finally {
      setalternateRouteBusy(false);
    }
  };

  const handleStartalternateRoute = async () => {
    const nkc = getNkcBridge();
    if (!nkc?.startalternateRoute) return;
    if (alternateRouteBusy) return;
    setalternateRouteBusy(true);
    try {
      ...
      console.error("Failed to start alternateRoute", error);
    } finally {
      setalternateRouteBusy(false);
    }
  };

  const handleStopalternateRoute = async () => {
    const nkc = getNkcBridge();
    if (!nkc?.stopalternateRoute) return;
    if (alternateRouteBusy) return;
    setalternateRouteBusy(true);
    try {
      await nkc.stopalternateRoute();
      await refreshalternateRouteStatus();
    } catch (error) {
      console.error("Failed to stop alternateRoute", error);
    } finally {
      setalternateRouteBusy(false);
    }
  };
  ...
  useEffect(() => {
    ...
    setTorBusy(false);
    setMyOnionAddress("");
    setalternateRouteStatus(null);
    setalternateRouteBusy(false);
    setalternateRouteProxyDraft("");
    setalternateRouteServiceDraft("");
    ...
    if (nkc?.getalternateRouteStatus) {
      nkc
        .getalternateRouteStatus()
        .then((value) => setalternateRouteStatus(value as alternateRouteStatus))
        .catch((e) => console.error("Failed to load alternateRoute status", e));
    }
    ...
    getalternateRouteExternalProxyUrl()
      .then((value) => setalternateRouteProxyDraft(value))
      .catch((e) => console.error("Failed to load alternateRoute proxy", e));
    getalternateRouteServiceAddress()
      .then((value) => setalternateRouteServiceDraft(value))
      .catch((e) => console.error("Failed to load alternateRoute service address", e));
    getOnionControllerUrlOverride()
      .then((value) => {
        setOnionControllerOverrideUrl(value);
  ...
  const canSaveOnion =
    !onionEnabledDraft ||
    (onionNetworkDraft === "tor" ? netConfig.tor.installed : netConfig.alternateRoute.installed);
            ...
            <div className="mt-6 grid gap-6">
                        ...
                        <div className="text-xs text-nkc-muted">
                          {opt.value === "directP2P"
                            ...
                            : opt.value === "onionRouter"
                              ? t("외부 Onion 경로(Tor/alternateRoute)", "External Onion route (Tor/alternateRoute)")
              ...
              <section className="rounded-nkc border border-nkc-border bg-nkc-panelMuted p-6">
                ...
                <div className="mt-3 text-xs text-nkc-muted">
                  ...
                  >
                    <option value="auto">Auto (alternateRoute then Tor)</option>
                    <option value="preferalternateRoute">Prefer alternateRoute</option>
              ...
              <section className="rounded-nkc border border-nkc-border bg-nkc-panelMuted p-6">
                <div className="text-sm font-semibold text-nkc-text">alternateRoute</div>
                <div className="mt-3 grid gap-3 text-xs text-nkc-muted">
                  <div>
                    Status: {alternateRouteStatus?.state ?? "unknown"}
                    {alternateRouteStatus?.details ? ` - ${alternateRouteStatus.details}` : ""}
                  </div>
                  {alternateRouteStatus?.proxyUrl ? (
                    <div>Proxy: {alternateRouteStatus.proxyUrl}</div>
                  ) : null}
                  <label className="text-xs text-nkc-muted">
                    External proxy (SOCKS)
                    <input
                      value={alternateRouteProxyDraft}
                      onChange={(e) => setalternateRouteProxyDraft(e.target.value)}
                  ...
                  <label className="text-xs text-nkc-muted">
                    ...
                    <input
                      value={alternateRouteServiceDraft}
                      onChange={(e) => setalternateRouteServiceDraft(e.target.value)}
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => void handleApplyalternateRouteExternal()}
                      disabled={alternateRouteBusy}
                      className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panel disabled:opacity-50"
                    >
                      Apply External alternateRoute
                    </button>
                    <button
                      type="button"
                      onClick={() => void handleStartalternateRoute()}
                      disabled={alternateRouteBusy}
                      className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panel disabled:opacity-50"
                    >
                      Start alternateRoute
                    </button>
                    <button
                      type="button"
                      onClick={() => void handleStopalternateRoute()}
                      disabled={alternateRouteBusy}
                      className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panel disabled:opacity-50"
                    >
                      Stop alternateRoute
                  ...
                  {onionControllerHealth ? (
                    <div className="text-xs text-nkc-muted">
                      ...
                      {onionControllerHealth.tor?.address
                        ? ` - ${onionControllerHealth.tor.address}`
                        : ""}
                      {onionControllerHealth.alternateRoute?.proxyUrl
                        ? ` - alternateRoute ${onionControllerHealth.alternateRoute.proxyUrl}`
                        : ""}
                      {onionControllerHealth.alternateRoute?.address
                        ? ` - ${onionControllerHealth.alternateRoute.address}`
                        : ""}
                      ...
                      <div className="rounded-nkc border border-nkc-border bg-nkc-panel px-4 py-3">
                        <div className="flex items-center justify-between">
                          <label className="flex items-center gap-2 text-sm font-medium text-nkc-text">
                            <input
                              type="radio"
                              name="onion-network"
                              checked={onionNetworkDraft === "alternateRoute"}
                              onChange={() => setOnionNetworkDraft("alternateRoute")}
                            />
                            alternateRoute{" "}
                          ...
                          <div className="text-xs text-nkc-muted">
                            {buildComponentLabel(netConfig.alternateRoute)}
                          </div>
                        </div>
                        <div className="mt-2 text-xs text-nkc-muted">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {!netConfig.alternateRoute.installed ? (
                            <button
                              type="button"
                              onClick={() => void handleInstall("alternateRoute")}
                            ...
                            >
                              {alternateRouteInstallBusy
                                ? t("처리 중...", "Working...")
                                : t(
                                    "alternateRoute 설치(관리자 권한 필요)",
                                    "Install alternateRoute (admin required)"
                                  )}
                            </button>
                          ) : null}
                          {netConfig.alternateRoute.installed &&
                          !(runtime?.network === "alternateRoute" && runtime?.status === "running") ? (
                            <button
                              type="button"
                              onClick={() => void handleConnectOnion("alternateRoute")}
                              disabled={alternateRouteInstallBusy || !isComponentReady(netConfig.alternateRoute)}
                              className="rounded-nkc bg-nkc-accent px-3 py-2 text-xs font-semibold text-nkc-bg disabled:opacity-50"
                            >
                              {t("연결", "Connect")}
                            </button>
                          ...
                          {runtime?.network === "alternateRoute" && runtime?.status === "running" ? (
                            <button
                              type="button"
                              onClick={() => void handleDisconnectOnion("alternateRoute")}
                              className="rounded-nkc border border-nkc-border px-3 py-2 text-xs text-nkc-text hover:bg-nkc-panelMuted"
                            >
                              {t("연결 해제", "Disconnect")}
                            </button>
                          ...
                          {alternateRouteUpdateAvailable ? (
                            <button
                              type="button"
                              onClick={() => void handleApplyUpdate("alternateRoute")}
                              disabled={alternateRouteApplyBusy}
                              className="rounded-nkc bg-nkc-accent px-3 py-2 text-xs font-semibold text-nkc-bg disabled:opacity-50"
                            >
                          ...
                          {netConfig.alternateRoute.installed ? (
                            <button
                              type="button"
                              onClick={() => void handleUninstall("alternateRoute")}
