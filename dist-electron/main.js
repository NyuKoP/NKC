...
const getalternateRoutePlatformLabel = (platform) => {
  switch (platform) {
    case "win32":
      return "win32";
    case "darwin":
...
const getalternateRouteArchLabel = (arch) => {
  switch (arch) {
    case "x64":
      return "amd64";
    case "ia32":
...
const getalternateRouteAssetExtension = (platform) => {
  switch (platform) {
    case "linux":
    case "darwin":
      return "tar.xz";
...
const getalternateRouteAssetName = (version, platform = process.platform, arch = process.arch) => `alternateRoute-${getalternateRoutePlatformLabel(platform)}-${getalternateRouteArchLabel(arch)}-v${version}.${getalternateRouteAssetExtension(
  platform
)}`;
const getalternateRouteAssetUrlForName = (version, assetName) => `${alternateRoute_RELEASE_BASE}/v${version}/${assetName}`;
...
const findalternateRouteBinaryWin32 = async () => {
  const candidates = [];
  await new Promise((resolve) => {
    node_child_process.execFile("where.exe", ["alternateRoute.exe"], { windowsHide: true }, (_error, stdout) => {
      if (stdout) {
        candidates.push(
  ...
  candidates.push(
    "C:\\\\Program Files\\\\alternateRoute\\\\alternateRoute.exe",
    "C:\\\\Program Files (x86)\\\\alternateRoute\\\\alternateRoute.exe",
    "C:\\\\Program Files\\\\alternateRoute\\\\alternateRoute\\\\alternateRoute.exe",
    "C:\\\\Program Files (x86)\\\\alternateRoute\\\\alternateRoute\\\\alternateRoute.exe"
  );
  ...
  const installLocationsRaw = await ps(
    "$paths=@(); $roots=@('HKLM:\\\\Software\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Uninstall\\\\*','HKLM:\\\\Software\\\\WOW6432Node\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Uninstall\\\\*'); foreach($r in $roots){   Get-ItemProperty $r -ErrorAction SilentlyContinue |     Where-Object { $_.DisplayName -match 'alternateRoute' -or $_.DisplayName -match 'alternateRoute' } |     ForEach-Object { if($_.InstallLocation){$paths+=$_.InstallLocation} } }; $paths | ForEach-Object { $_.Trim() } | Where-Object { $_ } | Select-Object -Unique | ForEach-Object { $_ }"
  );
  for (const line of installLocationsRaw.split(/\r?\n/).map((l) => l.trim()).filter(Boolean)) {
    candidates.push(path.join(line, "alternateRoute.exe"));
    candidates.push(path.join(line, "alternateRoute", "alternateRoute.exe"));
  }
  const servicePathRaw = await ps(
    "(Get-ItemProperty 'HKLM:\\\\SYSTEM\\\\CurrentControlSet\\\\Services\\\\alternateRoute' -ErrorAction SilentlyContinue).ImagePath"
  );
  const servicePath = servicePathRaw.trim().replace(/^\"|\"$/g, "");
  if (servicePath) {
    const exePath = servicePath.split(/\s+/)[0].replace(/^\"|\"$/g, "");
    candidates.push(exePath);
    candidates.push(path.join(path.dirname(exePath), "alternateRoute.exe"));
  }
...
const resolveDownload = (version, assetNameOverride) => {
  const assetName = assetNameOverride ?? getalternateRouteAssetName(version);
  return {
    assetName,
    url: getalternateRouteAssetUrlForName(version, assetName)
  };
};
const installalternateRoute = async (userDataDir, version, onProgress, downloadUrl, assetNameOverride) => {
  const network = "alternateRoute";
  const { assetName, url } = resolveDownload(version, assetNameOverride);
  const hash = getPinnedSha256(network, { version, assetName });
  if (!hash) {
    throw new PinnedHashMissingError(
      `Missing pinned hash for alternateRoute asset ${assetName} (${version}).`
    );
  }
  const baseOnionDir = path.join(userDataDir, "onion");
  await fs.mkdir(baseOnionDir, { recursive: true });
  ...
  onProgress?.({ step: "download", message: "Downloading alternateRoute" });
  try {
    await downloadFile(
      resolvedUrl,
      archivePath,
    ...
    const stat = await fs.stat(archivePath);
    details.downloadBytes = stat.size;
    onProgress?.({ step: "verify", message: "Verifying alternateRoute" });
    await verifySha256(archivePath, hash);
    details.expectedSha256 = hash;
    if (process.platform === "win32" && assetName.toLowerCase().endsWith(".exe")) {
      onProgress?.({ step: "activate", message: "Installing alternateRoute (requires admin)" });
      await runInstaller(archivePath);
      const found = await findalternateRouteBinaryWin32();
      if (!found) {
        throw new Error("BINARY_MISSING: alternateRoute.exe not found after installer");
      }
    ...
    onProgress?.({ step: "activate", message: "Activating alternateRoute" });
    const rollback = await swapWithRollback(userDataDir, network, { version, path: installPath });
    return { version, installPath, rollback };
  } catch (error) {
    if (error && typeof error === "object") {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error("[onion] alternateRoute install failed", { message, details });
    const wrapped = new Error(`${message} | details=${JSON.stringify(details)}`);
    wrapped.details = details;
    throw wrapped;
  } finally {
...
class OnionRuntime {
  ...
  alternateRouteManager = new alternateRouteManager();
  status = { status: "idle" };
  async start(userDataDir, network) {
    ...
    try {
      ...
      if (network === "tor") {
        ...
        await this.alternateRouteManager.start(binaryPath, port, dataDir);
      }
  ...
  async stop() {
    ...
    await this.alternateRouteManager.stop();
  ...
  getStatus() {
    return {
      ...
      alternateRoute: this.alternateRouteManager.getState()
    };
  }
}
...
const fetchReleases = async () => {
  const url = "https://api.github.com/repos/oxen-io/alternateRoute/releases?per_page=20";
...
const checkalternateRouteUpdates = async () => {
  const url = "https://api.github.com/repos/oxen-io/alternateRoute/releases/latest";
  ...
  if (!asset && process.platform === "win32") {
    ...
    for (const candidate of releases) {
      ...
      if (candidateAsset) {
        ...
        const sha2562 = getPinnedSha256("alternateRoute", {
  ...
  const sha256 = getPinnedSha256("alternateRoute", { version, assetName: asset.name });
  if (!sha256) {
    return {
      version,
...
const checkUpdates = async (network) => {
  ...
  return checkalternateRouteUpdates();
};
...
const registerOnionIpc = () => {
  ...
  electron.ipcMain.handle("onion:checkUpdates", async () => {
    ...
    const alternateRouteHasVerifiedUpdate = Boolean(alternateRouteUpdate.version && alternateRouteUpdate.sha256 && alternateRouteUpdate.downloadUrl);
    ...
    onionComponentCache.alternateRoute = {
      ...alternateRouteState,
      latest: alternateRouteHasVerifiedUpdate ? alternateRouteUpdate.version ?? void 0 : void 0,
      error: alternateRouteUpdate.errorCode === "PINNED_HASH_MISSING" ? "PINNED_HASH_MISSING" : void 0,
      detail: alternateRouteUpdate.errorCode === "PINNED_HASH_MISSING" ? `Pinned hash missing for ${alternateRouteUpdate.assetName ?? alternateRouteUpdate.version ?? "unknown"}` : void 0
    };
    return {
      components: {
